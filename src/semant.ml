...
let rec check_variable symbol_table s =
  ...
  with Not_found ->
    raise (Failure ("undeclared identifier " ^ string_of_var s))
...
and check_expr var_table func_table = function
  ...
  | VarExpr var -> (
      ...
      | VarIndex (v1, e) -> (
          let t, sx = check_expr var_table func_table (VarExpr v1) in
          match t with
          ...
          | List list_type ->
              ...
              if typvar = TypeVariable "int" then (list_type, SVarExpr var)
                ...
                raise
                  (Failure
                     ("Invalid indexing: " ^ string_of_var (VarIndex (v1, e))))
  ...
  | Dict dict_elems -> (
      ...
      | (k, v) :: _ ->
          ...
          let sdict_elems =
            check_dict var_table func_table dict_elems key_type value_type
          in
  ...
  | ListCompUnconditional (expr, loop_var, loop_list) -> (
      ...
      | List elem_type ->
          let expr_type, _ =
            check_expr
              (StringMap.add (string_of_var loop_var) elem_type var_table)
              func_table expr
          in
          ( List expr_type,
            SListCompUnconditional
              ( check_expr var_table func_table expr,
                loop_var,
                check_expr var_table func_table loop_list ) )
  ...
  | ListCompConditional (expr, loop_var, loop_list, condition) -> (
      ...
      | List elem_type ->
          ...
          let condition_type, _ =
            check_expr
              (StringMap.add (string_of_var loop_var) elem_type var_table)
              func_table condition
          in
          if condition_type != TypeVariable "bool" then
            raise
              (Failure
                 "List comprehension condition depends on non-boolean object")
          else
            ( List expr_type,
              SListCompConditional
                ( check_expr var_table func_table expr,
                  loop_var,
                  check_expr var_table func_table loop_list,
                  check_expr var_table func_table condition ) )
      | _ -> raise (Failure "List comprehension depends on non-list object"))
  | DictCompUnconditional (key_expr, value_expr, key_var, value_var, dict_expr)
    ->
      ...
      let updated_var_table =
        StringMap.add (string_of_var key_var) key_type var_table
      in
      let updated_var_table =
        StringMap.add (string_of_var value_var) value_type updated_var_table
      in
      let dict_expr_type, dict_sexpr_sx =
        check_expr updated_var_table func_table dict_expr
      in
      let dict_sexpr = (dict_expr_type, dict_sexpr_sx) in
      ( Dict (key_type, value_type),
        SDictCompUnconditional
          ( (key_type, key_sx),
            (value_type, value_sx),
            key_var,
            value_var,
            dict_sexpr ) )
  | DictCompConditional
      (key_expr, value_expr, key_var, value_var, dict_expr, condition) ->
      let key_type, key_sx = check_expr var_table func_table key_expr in
      let value_type, value_sx = check_expr var_table func_table value_expr in
      let updated_var_table =
        StringMap.add (string_of_var key_var) key_type var_table
      ...
      let dict_expr_type, dict_sexpr_sx =
        check_expr updated_var_table func_table dict_expr
      in
      let condition_type, condition_sexpr_sx =
        check_expr updated_var_table func_table condition
      in
      if condition_type != TypeVariable "bool" then
        raise
          (Failure "Dictionary comprehension condition must be of type bool");
  ...
  | FuncCall (fname, args) ->
      let return_type, func_name, sexpr_list =
        check_func_call var_table func_table fname args
      in
...
and list_to_sexpr list_elems symbol_table func_table head_type =
  ...
  | head :: tail -> (
      ...
      | t, _ ->
          if t != head_type then raise (Failure "Type mismatch in list")
            ...
            head_sexpr :: list_to_sexpr tail symbol_table func_table head_type)
...
and
  ...
  | head :: tail -> (
      ...
      match head_sexpr with
      | t, _ ->
          (List t, SList (list_to_sexpr list_elems symbol_table func_table t)))
  ...
  List.fold_left
    (fun acc (k, v) ->
      ...
      if k_type != key_type then
        raise
          (Failure
             ("Key type mismatch. Expected " ^ string_of_typevar key_type
            ^ ", found " ^ string_of_typevar k_type))
      else if v_type != value_type then
        raise
          (Failure
             ("Value type mismatch. Expected "
             ^ string_of_typevar value_type
             ^ ", found " ^ string_of_typevar v_type))
...
and binop_err_msg t1 t2 op =
  ...
  ^ " for binop " ^ (string_of_op op)

and check_binop e1 operation e2 symbol_table func_table =
...
and assignment_to_bop special_assignment =
...
and (*BlockAssign of variable * special_assignment * expr*)
  ...
  if assign_type != IdentityAssign then
    ...
    let binop_sexpr =
      check_binop (VarExpr lvalue) bop rvalue symbol_table func_table
    in
    match binop_sexpr with
    | t1, _ ->
        if t1 <> lvalue_type then
          raise
            (Failure
               ("Improperly typed assignment, lvalue: "
               ^ string_of_typevar lvalue_type
        ...
        if t1 <> lvalue_type then
          raise
            (Failure
               ...
               ^ string_of_typevar lvalue_type
               ^ ", rvalue: " ^ string_of_typevar t1))
        else SBlockAssign (lvalue, SAssign, rvalue_sexpr)

and check_func_types func_table func_name =
  try StringMap.find func_name func_table
  with Not_found -> raise (Failure ("undeclared identifier " ^ func_name))

and arg_list_helper symbol_table func_table params arg_types =
  ...
  | phead :: ptail, thead :: ttail -> (
      ...
      | t, _ ->
          if string_of_typevar t <> string_of_typevar thead then
            raise
              (Failure
                 ("Improperly typed parameter, should be: "
                ^ string_of_typevar t ^ " got: " ^ string_of_typevar thead))
          else
            phead_sexpr :: arg_list_helper symbol_table func_table ptail ttail)
  | _, _ -> raise (Failure "Wrong number of arguments")

and check_func_call symbol_table func_table func_name params =
...
and variable_declaration_helper symbol_table func_table var vartype =
  if StringMap.mem (string_of_var var) symbol_table then
    raise (Failure ("Variable " ^ string_of_var var ^ " already declared"))
  else if StringMap.mem (string_of_var var) func_table then
    raise
      (Failure
         ("Variable " ^ string_of_var var ^ " already declared as function"))
  else StringMap.add (string_of_var var) vartype symbol_table

and arg_to_type (arg_types : variable * typevar) =
  let _, t = arg_types in
  t

and function_declaration_helper symbol_table func_table func_name return_type
    args =
  if StringMap.mem (string_of_var func_name) func_table then
    raise
      (Failure ("Function " ^ string_of_var func_name ^ " already declared"))
  else if StringMap.mem (string_of_var func_name) symbol_table then
    raise
      (Failure
         ("Function " ^ string_of_var func_name
        ^ " already declared as function"))
  else
    StringMap.add (string_of_var func_name)
      (return_type, List.map arg_to_type args)
      func_table
...
and (* Return list of sblocks given blocks *)
    check_block_list allowed_block_set var_table func_table func_return_type =
  function
  | [] -> []
  | x :: xs ->
      let v_t, f_t, new_block_set, new_func_return_type, sblock =
        check_block allowed_block_set var_table func_table func_return_type x
      in
      sblock :: check_block_list new_block_set v_t f_t new_func_return_type xs

(* It returns a tuple of the var_table * func_table * Sast block *)
and add_func_params_to_var_table var_table func_table arg_list =
  match arg_list with
  | [] -> var_table
  | x :: xs ->
      let var_name, typ = x in
      let new_var_table =
        variable_declaration_helper var_table func_table var_name typ
      in
      add_func_params_to_var_table new_var_table func_table xs

and check_block allowed_block_set var_table func_table func_return_type =
  function
  | BlockAssign (v, s, e) ->
      ( var_table,
        func_table,
        allowed_block_set,
        func_return_type,
        check_assign var_table func_table v s e )
  | Break -> (
      match StringSet.find_opt "Break" allowed_block_set with
      | Some _ ->
          (var_table, func_table, allowed_block_set, func_return_type, SBreak)
      | None -> raise (Failure "Break must be specified within a loop."))
  | Continue -> (
      match StringSet.find_opt "Continue" allowed_block_set with
      | Some _ ->
          (var_table, func_table, allowed_block_set, func_return_type, SContinue)
  ...
  | ReturnVal expr -> (
      if func_return_type = None then
        raise (Failure "Return must be specified within a function.")
      else
                ...
                if t = func_return_type then
                  ...
                  raise
                    (Failure
                       ...
                       ^ string_of_typevar func_return_type
                       ^ " got " ^ string_of_typevar t))))
  | ReturnVoid -> (
      if func_return_type = None then
        raise (Failure "Return must be specified within a function.")
      else
        ...
        | Some _ ->
            if func_return_type = Some (TypeVariable "void") then
              ( var_table,
                func_table,
                allowed_block_set,
                func_return_type,
              ...
              raise (Failure "Expected no return expression in a void function")
      )
  | VarDec (typ, var, expr) ->
      let t, e = check_expr var_table func_table expr in
      if t = typ then
        let new_var_table =
          variable_declaration_helper var_table func_table var typ
        in
        ...
        raise
          (Failure
             ("Type mismatch, LHS: " ^ string_of_typevar typ ^ " RHS: "
            ^ string_of_typevar t))
  | While (loop_expr, loop_block_list) -> (
      ...
      | TypeVariable "bool", loop_sx ->
          let new_allowed_block_set =
            StringSet.add "Break" (StringSet.add "Continue" allowed_block_set)
          in
          let sblock_list =
            check_block_list new_allowed_block_set var_table func_table
              func_return_type loop_block_list
          in
          ( var_table,
            func_table,
            allowed_block_set,
            func_return_type,
            SWhile (loop_sexpr, sblock_list) )
  ...
  | For (loop_var, loop_expr, loop_block_list) -> (
      ...
      | List sub_type, loop_sx ->
          let new_var_table =
            StringMap.add (string_of_var loop_var) sub_type var_table
          in
          let new_allowed_block_set =
            StringSet.add "Break" (StringSet.add "Continue" allowed_block_set)
          in
          let sblock_list =
            check_block_list new_allowed_block_set new_var_table func_table
              func_return_type loop_block_list
          in
          ( var_table,
            func_table,
  ...
  | FuncBlockCall (fname, args) ->
      ...
      ( var_table,
        ...
        allowed_block_set,
        func_return_type,
        SFuncBlockCall (func_name, sexpr_list) )
  | FunctionDefinition (func_sig, block_list) ->
      let variable, arg_list, return_type = func_sig in
      let new_func_table =
        function_declaration_helper var_table func_table variable return_type
          arg_list
      in
      let new_var_table =
        add_func_params_to_var_table var_table func_table arg_list
      in
      let new_allowed_block_set =
        if return_type = TypeVariable "void" then
          StringSet.add "ReturnVoid" allowed_block_set
        else StringSet.add "ReturnVal" allowed_block_set
      in
      let sblock_list =
        check_block_list new_allowed_block_set new_var_table new_func_table
          (Some return_type) block_list
      in
      ( var_table,
        new_func_table,
        allowed_block_set,
        func_return_type,
        SFunctionDefinition (func_sig, sblock_list) )
  ...
  | IfNonEnd (expr, block_list, block) ->
      ...
      in
      let _, _, _, _, sblock =
        check_block allowed_block_set var_table func_table func_return_type
          block
      in
      ( var_table,
        func_table,
        allowed_block_set,
        func_return_type,
        SIfNonEnd (sexpr, sblock_list, sblock) )
  ...
  | ElifNonEnd (expr, block_list, block) ->
      ...
      ( var_table,
        ...
        allowed_block_set,
        func_return_type,
        SElifNonEnd (sexpr, sblock_list, sblock) )
  | ElseEnd block_list ->
      let sblock_list =
        check_block_list allowed_block_set var_table func_table func_return_type
          block_list
      ...
      ( var_table,
        ...
        allowed_block_set,
        func_return_type,
        SElseEnd sblock_list )
  | _ -> raise (Failure "Unsupported block type")

and check block_list =
  let builtin_entries =
    [
  ...
  let add_entry map (key, value) = StringMap.add key value map in

  let builtin_map = List.fold_left add_entry StringMap.empty builtin_entries in
