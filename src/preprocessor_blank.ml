...
let process_line define_map current_indent line line_num =
  let ignore_this =  all_spaces line in
  let indent_diff = String.length line - String.length (String.trim line) in
  let indent_change =
    if indent_diff > current_indent then
      if (indent_diff - current_indent) mod 4 != 0 then
        raise
          (Failure
             ("Indentation error in line " ^ string_of_int !line_num ^ ": "
            ^ line))
      else string_repeat ((indent_diff - current_indent) / 4) "INDENT"
    else if indent_diff < current_indent then
      String.concat "\n"
        (Array.to_list
           (Array.make ((current_indent - indent_diff) / 4) "DEDENT "))
      (*^ "\n"*)
    else ""
  in
  let updated_indent = indent_diff in
  let replace_x_with_y str (x, y) =
    Str.global_replace (Str.regexp_string x) y str
  in
  let line = List.fold_left replace_x_with_y line define_map in
  (indent_change, updated_indent, line, ignore_this)
...
let preprocess_file input_file output_file =
  ...
  try
    while true do
      ...
      | _ ->
          let indent_change, updated_indent, processed_line, ignore_line =
            process_line !define_map !current_indent line line_num
          in
          if not ignore_line then begin
  ...
  with End_of_file ->
    ...
    in
    output_string
      (open_out_gen [ Open_append; Open_creat ] 0o666 output_file)
      final_dedent
