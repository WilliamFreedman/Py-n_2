...
type variable =
  | Var of string
  (*| VarDot of variable * variable *)
  | VarIndex of variable * expr
...
and string_of_expr = function
  ...
  (** | IndexingVar(v, e) -> v ^ "[" ^ string_of_expr e ^ "]" **)
...
let rec string_of_typevar = function
  ...
  | Dict (t1, t2) ->
      "dict[" ^ string_of_typevar t1 ^ ", " ^ string_of_typevar t2 ^ "]"
