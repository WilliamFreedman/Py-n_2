...
type sassignment = SAssign

type svariable = variable
(** Variable / term indicator, with dots and indexing **)
(*| SVarDot of svariable * svariable | SVarIndex of svariable * sexpr *)
...
and sx =
  ...
  | SDictCompConditional of
      sexpr * sexpr * svariable * svariable * sexpr * sexpr
  ...
  ^ (match e with
    ...
    | SDict d ->
        let expr_expr_printer = function
          | e1, e2 -> string_of_sexpr e1 ^ " : " ^ string_of_sexpr e2
        in
    ...
    | SFuncCall (v, e) ->
        ...
        ^ ")"
    | SListCompUnconditional (e1, v, e2) ->
        "[" ^ string_of_sexpr e1 ^ " for " ^ string_of_svar v ^ " in "
        ^ string_of_sexpr e2 ^ "]"
    | SListCompConditional (e1, v, e2, e3) ->
        "[" ^ string_of_sexpr e1 ^ " for " ^ string_of_svar v ^ " in "
        ^ string_of_sexpr e2 ^ " if " ^ string_of_sexpr e3 ^ "]"
    (** Shouldn't be needed anymore: | IndexingVar(v, e) -> v ^ "[" ^ string_of_expr e ^ "]" **)
    | SIndexingStringLit (s, e) -> s ^ "[" ^ string_of_sexpr e ^ "]"
    | SIndexingExprList (e1, e2) ->
        string_of_sexpr e1 ^ "[" ^ string_of_sexpr e2 ^ "]"
    | SDictCompConditional (e1, e2, v1, v2, e3, e4) ->
        "{" ^ string_of_sexpr e1 ^ " : " ^ string_of_sexpr e2 ^ " for ("
        ^ string_of_svar v1 ^ ", " ^ string_of_svar v2 ^ ") in "
        ^ string_of_sexpr e3 ^ "if" ^ string_of_sexpr e4 ^ "}"
    | SDictCompUnconditional (e1, e2, v1, v2, e3) ->
        "{" ^ string_of_sexpr e1 ^ " : " ^ string_of_sexpr e2 ^ " for ("
        ^ string_of_svar v1 ^ ", " ^ string_of_svar v2 ^ ") in "
        ^ string_of_sexpr e3 ^ "}")
  ^ ")"
...
let string_of_sfunc_sig = function
  | v, args, ret_type ->
      "def " ^ string_of_svar v ^ "("
      ^ String.concat ", " (List.map string_of_sarg args)
...
let rec string_of_sblock = function
  ...
  | SClassDefinitionImplements (v, interfaces, block_list) ->
      "class " ^ string_of_svar v ^ " implements "
      ^ String.concat ", " (List.map string_of_svar interfaces)
      ^ ":\n"
      ^ String.concat "\n" (List.map string_of_sblock block_list)
      ^ "\n"
  | SIfEnd (e, bl) ->
